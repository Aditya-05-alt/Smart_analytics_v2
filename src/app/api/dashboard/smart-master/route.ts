...
>(qb: T, from: string, to: string, reportBefore: string | null): T {
...
async function fetchSmartMasterRows(
  db: SupabaseClient,
  from: string,
  to: string,
  ...
  for (let offset = 0; offset < MAX_ROWS; offset += PAGE_SIZE) {
    const { data, error } = await applyReportDateRange(
...
export async function GET(request: Request) {
  ...
  try {
    ...
    let { rows, error: err1 } = await fetchSmartMasterRows(
      db,
      from,
      to,
      reportBefore,
    ...
    if (rows.length === 0 && cid) {
      const { rows: byClient, error: err2 } = await fetchSmartMasterRows(
        db,
        from,
        to,
        reportBefore,
